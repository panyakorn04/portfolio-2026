...
export function PortfolioShell({
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto grid max-w-[92rem] gap-4 px-4 py-4 sm:px-6 lg:grid-cols-[17rem_minmax(0,1fr)] lg:py-6 xl:grid-cols-[17rem_minmax(0,1fr)_20rem]">
        <aside className="hidden lg:block">
          <div className={`${panelClass} sticky top-24 overflow-hidden p-3`}>
            <div className="rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-[var(--surface)] p-3">
              <div className="flex items-center gap-3">
                <span className="grid size-10 shrink-0 place-items-center rounded-[0.7rem] bg-[linear-gradient(135deg,var(--color-accent),#16a34a)] font-mono text-xs font-black text-[#05070a] shadow-[0_0_24px_var(--accent-glow)]">
                  PB
                </span>
                <div className="min-w-0">
                  <p className={eyebrowClass}>portfolio os</p>
            ...
            <nav className="mt-3 grid gap-1.5">
              {dictionary.navItems.map((item, index) => (
                <a
                  key={item.id}
                  href={`/${locale}#${item.id}`}
                  className="group flex items-center justify-between rounded-[var(--radius-sm)] border border-transparent px-3 py-2.5 text-sm text-[var(--color-muted)] transition-colors hover:border-[var(--color-line)] hover:bg-[var(--surface-hover)] hover:text-[var(--color-text)]"
                >
                  <span>{item.label}</span>
                  <span className="font-mono text-[0.58rem] text-[var(--color-soft)] group-hover:text-[var(--color-accent)]">
                    0{index + 1}
                  </span>
                </a>
              ))}
              <Link
                ...
                className="group flex items-center justify-between rounded-[var(--radius-sm)] border border-transparent px-3 py-2.5 text-sm text-[var(--color-muted)] transition-colors hover:border-[var(--color-line)] hover:bg-[var(--surface-hover)] hover:text-[var(--color-text)]"
              >
                <span>{articleCopy.navLabel}</span>
                <span className="font-mono text-[0.58rem] text-[var(--color-soft)] group-hover:text-[var(--color-accent)]">
                ...
                <div className="grid gap-6 p-5 sm:p-6 xl:grid-cols-[minmax(0,1fr)_17rem]">
                  ...
                  <div className="grid content-start gap-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 xl:grid-cols-1">
                      {dictionary.stats.slice(0, 2).map((stat) => (
          ...
          >
            <div className="grid gap-4 lg:grid-cols-[16rem_minmax(0,1fr)]">
              <div className={`${compactPanelClass} p-4`}>
                ...
                <p className={`${smallCopyClass} mt-2`}>{sections.timeline}</p>
                  ...
                  <div className="mt-4 grid gap-3">
                    {[
                      ...
                      >
                        ...
                        <p className={`${smallCopyClass} mt-2 text-pretty`}>
                          {item.help}
                        </p>
        ...
        <aside className="hidden xl:block">
          <div className="sticky top-24 grid gap-4">
            <div className={`${panelClass} p-4`}>
              <p className={eyebrowClass}>{ui.statsLabel}</p>
              <div className="mt-3 grid gap-3">
                {dictionary.stats.map((stat) => (
                  <div
                    key={stat.value}
                    className="rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-[var(--surface)] p-3"
                  >
                    <p className="text-3xl font-semibold tracking-[-0.06em] text-[var(--color-accent)]">
                      {stat.value}
                    </p>
                    <p className={`${smallCopyClass} mt-1`}>{stat.label}</p>
                  </div>
            ...
            <div className={`${panelClass} p-4`}>
              <p className={eyebrowClass}>{ui.linksLabel}</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {socialLinks.map((link) => (
                  <a
                    key={link.key}
