  ...
  return (
    <section id={id} className="scroll-mt-[6.5rem] py-12 lg:py-16">
      <div className="border-t border-[rgba(111,247,166,0.12)] pt-6 lg:pt-8">
        ...
        <h2 className={`${sectionTitleClass} mt-4 max-w-4xl text-balance`}>{title}</h2>
        {text ? (
          <p className={`${sectionCopyClass} mt-4 max-w-3xl text-pretty`}>{text}</p>
        ) : null}
      </div>
  ...
  return (
    ...
    >
              ...
              <div className="relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className={terminalWindowBarClass}>
                    <span className={`${terminalDotClass} bg-[#ff5f56]`} />
                    <span className={`${terminalDotClass} bg-[#ffbd2e]`} />
                    <span className={`${terminalDotClass} bg-[#27c93f]`} />
                  ...
                  <div className="space-y-4 p-4 sm:p-5 lg:p-7">
                    ...
                    <div>
                      <PromptLine command={commands.role} value={hero.badge} />
                    </div>
                    ...
                    <div className="pt-3">
                      ...
                      <p className={`${sectionCopyClass} mt-5 max-w-2xl text-pretty`}>
                        {hero.heroText}
                      </p>
                    </div>

                    <div className="grid gap-3 border-t border-[var(--color-line)] pt-4 sm:grid-cols-2">
                      <PromptLine command={commands.focus} value={hero.focusedText} />
                      <PromptLine
          ...
          <aside className="space-y-4 self-start lg:sticky lg:top-[5rem]">
            <MotionReveal className="rounded-2xl">
              <div className={terminalPanelClass}>
                <p className={terminalLabelClass}>{ui.profilePreviewLabel}</p>
            ...
            <MotionReveal className="rounded-2xl">
              <div className={terminalPanelClass}>
                <p className={terminalLabelClass}>{ui.statsLabel}</p>
                <div className="mt-3 divide-y divide-[rgba(111,247,166,0.07)]">
                  {dictionary.stats.map((stat) => (
                    <div key={stat.value} className="py-3 first:pt-0 last:pb-0">
            ...
            <MotionReveal className="rounded-[1.45rem]">
              <div className={terminalPanelClass}>
                <p className={terminalLabelClass}>{ui.linksLabel}</p>
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-12">
            ...
            {sections.principles.map((item, index) => (
              <MotionReveal key={item} className="rounded-2xl">
                <article className={terminalPanelClass}>
                  <p className={terminalLabelClass}>rule_0{index + 1}</p>
                  <p className={`${terminalCopyClass} mt-3 text-pretty`}>{item}</p>
                </article>
              </MotionReveal>
            ))}
          </div>
        ...
        >
          <div className="space-y-5">
            {sections.featuredWork.map((project, index) => (
              <MotionReveal key={project.title} className="rounded-2xl">
                <article
                ...
                >
                  <div className="grid gap-5 lg:grid-cols-[11rem_minmax(0,1fr)_11rem] lg:gap-7">
                    <div className="space-y-2 border-b border-[rgba(111,247,166,0.08)] pb-3 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-5">
                      <p className={terminalLabelClass}>case_0{index + 1}</p>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className={`${terminalCopyClass} mt-3 max-w-2xl text-pretty`}>
                        {project.description}
                      </p>
                      <ul className={`${terminalCopyClass} mt-4 space-y-2.5`}>
                    ...
                    <div className="border-t border-[rgba(111,247,166,0.08)] pt-3 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-5">
                      <p className={terminalLabelClass}>{ui.stackLabel}</p>
                      <div className="mt-2.5 flex flex-wrap gap-2">
                        {project.stack.map((item) => (
                          <span key={item} className={terminalChipClass}>
                            {item}
                          </span>
                        ))}
                      </div>
        ...
        >
          <MotionReveal className="rounded-2xl">
            <div className={`${terminalPanelClass} overflow-hidden`}>
              <div className="divide-y divide-[rgba(111,247,166,0.07)]">
                {sections.skills.map((item) => (
                  ...
                  >
                    <p className={`${terminalLabelClass} mb-3 pt-0.5 lg:mb-0`}>
                      {item.group}
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {item.list.split(", ").map((skill) => (
                        <span key={skill} className={terminalChipClass}>
                          {skill.trim()}
                        </span>
                      ))}
                    </div>
        ...
        >
          <div className="grid gap-5 lg:grid-cols-[14rem_minmax(0,1fr)] lg:gap-6">
            <MotionReveal className="rounded-2xl">
              <div className={terminalPanelClass}>
                <p className={terminalLabelClass}>{ui.currentRoleLabel}</p>
            ...
            <MotionReveal className="rounded-2xl">
              <div className={terminalPanelClass}>
                <LocalizedBullets items={sections.experienceBullets} />
              </div>
            </MotionReveal>
          </div>
        </SectionBlock>
            ...
            <div className="space-y-5">
              <div className="grid gap-4 lg:grid-cols-2">
                {articles.map((article) => (
                  <MotionReveal key={article.slug} className="rounded-2xl">
                    <Link
